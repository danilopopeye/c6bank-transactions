...
- [ ] 1.2 Add exported `Deduplicate(transactions []Transaction) []Transaction` function that removes duplicates using FNV-1a hash on Date+Payee+Amount
...
- [ ] 2.2 Implement main logic: iterate over file args, call `ParseFile` for each, accumulate results, deduplicate, write CSV
